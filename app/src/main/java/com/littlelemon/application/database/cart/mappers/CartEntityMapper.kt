...
fun List<CartItemDetails>.toCartDetailItems(): List<CartDetailItem> {
    return map { (cartItem, dishEntity) ->
        CartDetailItem(dishEntity.toDish(), cartItem.quantity)
    }
}

fun List<CartItemEntity>.toCartItems(): List<CartItem> = map { cartItemEntity ->
    CartItem(dishId = cartItemEntity.dishId, quantity = cartItemEntity.quantity)
}